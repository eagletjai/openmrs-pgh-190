				...
				if (!model.containsKey("patient")) {
					...
					if (Context.hasPrivilege(PrivilegeConstants.VIEW_PATIENTS)) {
						...
						if (Context.hasPrivilege(PrivilegeConstants.VIEW_ENCOUNTERS)) {
							...
							log.debug("MetaHealthcare PortletController.handleRequest");
							
							Set<Role> currentUserRoles = Context.getAuthenticatedUser().getRoles();
							HashMap<Integer, HashMap<String, String>> rolePermissionsByFormId = Context.getFormService()
							        .getRolePermissions();
							
							encs = Context.getEncounterService().getEncountersByPatient(p);
							int ix = 0;
							for (Encounter enc : encs) {
								HashMap<String, String> formRolePermissions = rolePermissionsByFormId.get(enc.getForm()
								        .getFormId());
								//								log.debug("MetaHealthcare: form.id " + enc.getForm().getId());
								//								log.debug("MetaHealthcare: formId " + enc.getForm().getFormId());
								//								log.debug("MetaHealthcare: form.name " + enc.getForm().getName());
								//								log.debug("MetaHealthcare: formRolePermissions for form " + enc.getForm().getFormId() + ": "
								//								        + formRolePermissions.size());
								boolean canEdit = false;
								boolean canView = false;
								// if no permissions are stored for this form, deny everything
								if (formRolePermissions == null || formRolePermissions.size() == 0) {
									log.debug("MetaHealthcare: no role permissions for this form, so grant for all! "
									        + enc.getForm().getFormId());
									canEdit = false;
									canView = false;
								} else {
									// loop through form permissions
									for (Iterator<String> iter = formRolePermissions.keySet().iterator(); iter.hasNext();) {
										String key = iter.next();
										log.debug("MetaHealthcare: formRolePermissions checking key " + key);
										// check all user roles and see whether form permissions are assigned to this role
										for (Role role : currentUserRoles) {
											log.debug("MetaHealthcare: formRolePermissions checking key " + key + " vs. "
											        + role.getRole());
											if (key.equalsIgnoreCase(role.getRole())) {
												String roleType = formRolePermissions.get(key);
												// check role type and set permission flag
												if (roleType.indexOf(PERMISSION_EDIT) >= 0) {
													canEdit = true;
												}
												if (roleType.indexOf(PERMISSION_VIEW) >= 0) {
													canView = true;
												}
											}
										}
								...
								enc.setCanEdit(canEdit);
								enc.setCanView(canView);
								ix++;
							}
						} else {
							encs = new ArrayList<Encounter>();
						}
						model.put("patientEncounters", encs);
						// end: Meta Healthcare
						...
						*/
						...
						// TODO: Meta Healthcare: check if any customization is required here for 1.9.0
