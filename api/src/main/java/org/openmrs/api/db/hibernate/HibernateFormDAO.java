...
public class HibernateFormDAO implements FormDAO {
	...
	/**
	 * Get role permissions for all forms.
	 * @return Hash map that holds permission info, with role name as key.
	 * @throws DAOException
	 */
	public HashMap<Integer, HashMap<String, String>> getRolePermissions() throws DAOException {
		String query = "select htmlformentry_html_form.form_id AS id,htmlformentry_html_form_role.role AS role,htmlformentry_html_form_role.role_type AS roleType "
		        + "from htmlformentry_html_form_role,htmlformentry_html_form "
		        + "where htmlformentry_html_form_role.html_form_id =  htmlformentry_html_form.id "
		        + "order by htmlformentry_html_form_role.html_form_id, htmlformentry_html_form_role.role";
		
		log.info("MetaHealthcare Query " + query);
		
		List rolePermissions = sessionFactory.getCurrentSession().createSQLQuery(query).addScalar("id").addScalar("role")
		        .addScalar("roleType").list();
		
		log.info("MetaHealthcare rolePermissions.size " + rolePermissions.size());
		
		HashMap<Integer, HashMap<String, String>> mapByForm = new HashMap<Integer, HashMap<String, String>>();
		for (Iterator rolePermissionsIterator = rolePermissions.iterator(); rolePermissionsIterator.hasNext();) {
			Object[] tuple = (Object[]) rolePermissionsIterator.next();
			
			log.info("MetaHealthcare tuple " + tuple);
			
			if (mapByForm.get(tuple[0]) == null) {
				mapByForm.put((Integer) tuple[0], new HashMap<String, String>());
			}
			log.info("MetaHealthcare putting " + tuple[0] + ": " + tuple[1] + " - " + tuple[2]);
			mapByForm.get(tuple[0]).put((String) tuple[1], (String) tuple[2]);
